...
const ShowProductsContent = ({ view }: { view: "list" | "grid" }) => {
  ...
  // fetch from the route /api/products
  //   useEffect(() => {
  //   let ignore = false;
  ...
  //         `/api/products?page=${page}&search=${search}&filter=${encodeURIComponent(
  ...
  //       setProducts((prev) => (page === 0 ? formatted : [...prev, ...formatted]));
  //       setHasMore(data.length >= 24);
  //     } catch (error) {
  //       console.error("❌ Error fetching products:", error);
  //     } finally {
  ...
  //       const query = filteredProductsQuery(page, search, productFilter);
  //       console.log("the query to search with is : ", query);
  //       // const data = await client.fetch(query);
  //       // const data = await client.fetch(query, {}, { cache: "no-store" });
  //       const data = await client.fetch(query, {}, { next: { revalidate: 0 } });

  //       console.log("the data in the show products is : ", data);
  //       if (ignore) return;
  ...
  //         };
  //         console.log(
  //           "the final return in the show products is : ",
  //           finalReturn
  //         );
  //         return finalReturn;
  //       });
