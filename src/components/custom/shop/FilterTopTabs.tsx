...
type Item = {
  text: string;
  key: string; // filter key (e.g. audience, season)
  value: string; // filter value (e.g. men, summer)
};
  ...
  return (
    <div className={`relative w-full min-w-0 ${className}`}>
      <div className="overflow-hidden w-full" ref={emblaRef}>
        <div className="embla__container py-1 px-2 flex gap-x-3">
          {items.map((item, i) => (
            <Button
              key={i}
              onClick={() => handleFilterClick(item.key, item.value)}
              size="lg"
              variant={selectedFilter === item.value ? "default" : "outline"}
              className={`flex-shrink-0 hover:scale-105 transition-all`}
            >
              {item.text}
            </Button>
      ...
      {canScrollPrev && (
        <button
          onClick={() => emblaApi?.scrollPrev()}
          className="absolute top-1/2 left-2 -translate-y-1/2 hover:scale-110 transition"
        >
          <ChevronLeft className="w-6 md:w-7 h-7 md:h-7 text-primary-foreground" />
        </button>
      )}
      {canScrollNext && (
        <button
          onClick={() => emblaApi?.scrollNext()}
          className="absolute top-1/2 right-2 -translate-y-1/2 hover:scale-110 transition"
        >
          <ChevronRight className="w-6 md:w-7 h-7 md:h-7 text-primary-foreground" />
        </button>
      )}
